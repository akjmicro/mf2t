...
char **argv;
{
  ...
  if (arg_index < argc && !freopen(argv[arg_index], "w", stdout))
...
FILE *efopen(name, mode) char *name;
char *mode;
{
  FILE *f;
  char *errmess;
...
char *mknote(pitch) int pitch;
{
  static char *Notes[] = {"c",  "c#", "d",  "d#", "e",  "f",
                          "f#", "g",  "g#", "a",  "a#", "b"};
  static char buf[5];
  if (notes)
...
myheader(format, ntrks, division) int format, ntrks, division;
{
  if (division & 0x8000) { /* SMPTE */
    times = 0;             /* Can't do beats */
    printf("MFile %d %d %d %d\n", format, ntrks, -((-(division >> 8)) & 0xff),
           division & 0xff);
  } else
...
mynon(chan, pitch, vol) int chan, pitch, vol;
{
  prtime();
  printf(Onmsg, chan + 1, mknote(pitch), vol);
}

mynoff(chan, pitch, vol) int chan, pitch, vol;
{
  prtime();
  printf(Offmsg, chan + 1, mknote(pitch), vol);
}

mypressure(chan, pitch, press) int chan, pitch, press;
{
  prtime();
  printf(PoPrmsg, chan + 1, mknote(pitch), press);
}

myparameter(chan, control, value) int chan, control, value;
{
  prtime();
  printf(Parmsg, chan + 1, control, value);
}

mypitchbend(chan, lsb, msb) int chan, lsb, msb;
{
  prtime();
  printf(Pbmsg, chan + 1, 128 * msb + lsb);
}

myprogram(chan, program) int chan, program;
{
  prtime();
  printf(PrChmsg, chan + 1, program);
}

mychanpressure(chan, press) int chan, press;
{
  prtime();
  printf(ChPrmsg, chan + 1, press);
}

mysysex(leng, mess) int leng;
char *mess;
{
  prtime();
  printf("SysEx");
...
mymmisc(type, leng, mess) int type, leng;
char *mess;
{
  prtime();
...
mymtext(type, leng, mess) int type, leng;
char *mess;
{
  static char *ttype[] = {NULL,        "Text",             /* type=0x01 */
                          "Copyright",                     /* type=0x02 */
                          "TrkName",   "InstrName",        /* ...       */
                          "Lyric",     "Marker",    "Cue", /* type=0x07 */
                          "Unrec"};
...
mytempo(tempo) long tempo;
{
  prtime();
  printf("Tempo %ld\n", tempo);
}

mytimesig(nn, dd, cc, bb) int nn, dd, cc, bb;
{
  int denom = 1;
  while (dd-- > 0)
    denom *= 2;
...
mysmpte(hr, mn, se, fr, ff) int hr, mn, se, fr, ff;
{
  prtime();
  printf("SMPTE %d %d %d %d %d\n", hr, mn, se, fr, ff);
}

myarbitrary(leng, mess) int leng;
char *mess;
{
  prtime();
  printf("Arb", leng);
...
prtime() {
  if (times) {
    ...
    printf("%ld:%ld:%ld ", m / Measure + M0, m % Measure,
           (Mf_currtime - T0) % Beat);
  } else
    printf("%ld ", Mf_currtime);
}

prtext(p, leng) unsigned char *p;
int leng;
{
  int n, c;
  int pos = 25;
